...
__all__ = ["DataloaderSubjectAdaptiveTransfer"]


class DataloaderSubjectAdaptiveTransfer(DataloaderBase):
    """
    ...
    def setup(self, stage: str = None, k: int = 1, mode: str = "pretrain"):
        """
        Setup for the dataloader.

        :param stage: setup dataloader for train/val/test sets. Default: None.
        :param k: the session index. Default: 2
        :param mode: training mode for transfer learning:
            * "pretrain": running the pretraining phase.
            * "transfer": running the transferring/fine-tuning phase.
        """
        if stage in [None, "fit"]:
            data_train = np.load(str(self.data_root / f"{mode}_train_fold{k}.npz"), allow_pickle=True)
            data_val = np.load(str(self.data_root / f"{mode}_val_fold{k}.npz"), allow_pickle=True)
            ...
            data_test = np.load(str(self.data_root / f"{mode}_test{k}.npz"), allow_pickle=True)

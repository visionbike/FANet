...
# supports group learning rates (thanks @SHolderbach), fixes sporadic load from saved network issues.
...
class Ranger20(Optimizer):
    ...
    * https://github.com/lessw2020/Ranger-Deep-Learning-Optimizer
    ...
    def __init__(self,
        ...
        :param use_gc: whether to use Gradient Centralization (GC) or not. Default: True.
        :param gc_conv_only: whether to apply GC to conv1d layers only or conv1d + fc layers. Default: False.
        :param gc_loc: for adaptive learning rate methods. Default: False.
        """
        # parameter checks
        if alpha < 0.0 or alpha > 1.0:
            raise ValueError(f'Invalid slow update rate, but got {alpha}.')
        if k > 1:
            raise ValueError(f'Invalid lookahead steps, but got {k}.')
        if lr < 0.0:
            raise ValueError(f'Invalid LR value, but got {lr}.')
        if eps < 0.0:
            raise ValueError(f'Invalid epsilon value, but got {eps}.')
        # prep defaults and init torch.optim base
        defaults = dict(lr=lr, alpha=alpha, k=k, step_counter=0, betas=betas, N_sma_threshhold=n_sma_threshold, eps=eps, weight_decay=weight_decay)
